%% Assignment/Project Script Test File : MACHINE_PATTERN_NYSEOLDDATACOMMEKINAR_001aa.m
...
stock2 = csvread('kinar.csv');

% x is the return vector of the stocks
x = [stock1(:,2) stock2(:,2)];
dates =  stock1(:,1);
dates = datenum(strcat('19', int2str(dates)),'yyyymmdd');
%% 6. Active Portfolios
...
% active
save(strcat(fullfile(userpathstr,projectpath,'scripts\NYSEold\'),'comme_kinar_active.mat'),'p2');
% absolute
save(strcat(fullfile(userpathstr,projectpath,'scripts\NYSEold\'),'comme_kinar_absolute.mat'),'p3')

%% 9. Print Ouput
% plot the required output
figure;
plotVar = {'p2.S','cumsum(transpose(squeeze(x2))-1)','p2.SH','p2.b'};
plotTitle = {'Case : NYSEOLD commekinar'};
plotX = {'time'};
plotY = {'S','x2','SH','b'};
for i=1:4
    subplot(2,2,i);
