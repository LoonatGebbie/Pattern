...
stock1 = csvread('ibm.csv');
stock2 = csvread('coke.csv');
...
% active
save(strcat(fullfile(userpathstr,projectpath,'scripts\NYSEold\'),'ibm_coke_active.mat'),'p2');
% absolute
save(strcat(fullfile(userpathstr,projectpath,'scripts\NYSEold\'),'ibm_coke_absolute.mat'),'p3')

%% 9. Print Ouput
% % plot the required output
% figure;
% plotVar = {'p2.S','cumsum(transpose(squeeze(x2))-1)','p2.SH','p2.b'};
% plotTitle = {'Case : NYSEOLD ibmcoke'};
% plotX = {'time'};
% plotY = {'S','x2','SH','b'};
% for i=1:4
%     subplot(2,2,i);
